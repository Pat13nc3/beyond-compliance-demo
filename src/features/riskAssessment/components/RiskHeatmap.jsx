    ...
    return (
        <div className="overflow-x-auto">
            <table className="min-w-full border-collapse">
                <thead>
                    <tr>
                        <th className="p-3 font-medium text-left text-sm text-gray-500 border-b">Product</th>
                        {riskData.jurisdictions.map(j => (
                            <th key={j} className="p-3 font-medium text-center text-sm text-gray-500 border-b">{j}</th>
                        ))}
                    </tr>
                </thead>
                <tbody>
                    {riskData.products.map((product, rowIndex) => (
                        <tr key={product}>
                            <td className="p-3 border-b border-gray-200 font-semibold">{product}</td>
                            {riskData.scores[rowIndex].map((score, colIndex) => (
                                <td key={`${rowIndex}-${colIndex}`} className="p-1 border-b border-gray-200">
                                    <div
