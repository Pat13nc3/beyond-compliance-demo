...
import { X, Save, Puzzle } from 'lucide-react';

const NewIntegrationModal = ({ onClose, onSave }) => {
    const [integrationName, setIntegrationName] = useState('');
    const [integrationType, setIntegrationType] = useState('KYC/AML');

    const handleSave = () => {
        if (!integrationName) {
            alert('Please provide a name for the integration.');
            return;
        }
        onSave({ name: integrationName, type: integrationType });
        onClose();
    };
        ...
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 animate-fade-in-fast">
            <div className="bg-[#1e252d] rounded-2xl shadow-2xl p-6 w-full max-w-lg text-white">
                <div className="flex justify-between items-center mb-4 border-b border-gray-700 pb-3">
                    <h3 className="text-2xl font-bold text-[#c0933e]">Add New Integration</h3>
                ...
                <div className="space-y-4">
                    <div>
                        ...
                        <input
                            ...
                            placeholder="e.g., New KYC Provider"
                    ...
                    <div>
                        ...
                        <select
                            ...
                            className="w-full p-2 border border-gray-600 rounded-md bg-gray-800 text-white focus:ring-[#c0933e] focus:border-[#c0933e]"
                        >
                            <option>KYC/AML Services</option>
                            <option>Regulator Platforms</option>
                            <option>Partner Collaboration</option>
                            <option>Audit Management</option>
                        </select>
                    </div>
                     <div>
                        <label className="block text-sm font-medium text-gray-300 mb-2">Configuration Details</label>
                        <textarea
                            rows={4}
                            placeholder="Enter setup instructions or API endpoint details here..."
                            className="w-full p-2 border border-gray-600 rounded-md bg-gray-800 text-white focus:ring-[#c0933e] focus:border-[#c0933e]"
                        />
                    </div>
                </div>
                <div className="flex justify-end pt-6 mt-6 border-t border-gray-700">
                    ...
                    <button type="button" onClick={handleSave} className="bg-blue-600 text-white font-bold py-2 px-4 rounded-md text-sm hover:bg-blue-500 flex items-center">
                        <Save size={16} className="mr-2"/> Save Integration
                    </button>
                </div>
            </div>
        </div>
