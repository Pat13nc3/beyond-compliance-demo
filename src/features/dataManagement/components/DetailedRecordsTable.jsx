...
// NEW: Component for a single data set card
const DataSetCard = ({ icon, title, description, filter, active, onClick }) => (
    <button onClick={() => onClick(filter)} className={`theme-bg-card p-6 rounded-xl border-2 transition-all duration-200 text-left h-full flex flex-col justify-between ${active ? 'border-blue-500 shadow-lg' : 'border-transparent hover:border-blue-500 hover:shadow-md'}`}>
        <div className="flex items-center space-x-4">
            <div className={`p-3 rounded-lg ${active ? 'bg-blue-500/20' : 'bg-gray-100 dark:bg-gray-700'}`}>
                {icon}
            </div>
            <div>
...
const DetailedRecordsTable = ({ initialFilters = {}, triggerAIAnalysis }) => {
    ...
    const dataSetOptions = [
        { id: 'kyc', title: 'Customer KYC Records', description: 'Comprehensive Know Your Customer data for all clients.', icon: <Database size={24} className="text-blue-500" />, filter: { type: 'KYC', source: 'All' } },
        { id: 'payments', title: 'Payment Transactions', description: 'All incoming and outgoing payment transaction logs.', icon: <CreditCard size={24} className="text-green-500" />, filter: { type: 'Payment', source: 'All' } },
        { id: 'aml', title: 'AML Alert Logs', description: 'Records of all Anti-Money Laundering alerts generated by the system.', icon: <ShieldAlert size={24} className="text-red-500" />, filter: { type: 'AML Alert', source: 'All' } },
        { id: 'user', title: 'User Activity Logs', description: 'Detailed logs of user interactions and system access.', icon: <User size={24} className="text-purple-500" />, filter: { type: 'User', source: 'All' } },
        { id: 'sanctions', title: 'Sanctions Screening Results', description: 'Results from sanctions and watchlist screenings.', icon: <CheckCircleIcon size={24} className="text-orange-500" />, filter: { type: 'Sanctions Check', source: 'All' } },
    ];
    
    // Derived state to keep filter selects in sync with card clicks
    const handleCardClick = (filter) => {
      setCurrentDataTypeFilter(filter.type);
      setCurrentSourceFilter(filter.source);
    ...
    return (
        <div className="p-6 theme-bg-page min-h-screen theme-text-primary">
            <div className="max-w-7xl mx-auto">
                ...
                {/* NEW: Data Set Cards */}
                <div className="mb-8">
                    <h2 className="text-2xl font-bold theme-text-primary mb-4">Explore Data Sets</h2>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {dataSetOptions.map(dataSet => (
                            <DataSetCard
                                ...
                                active={currentDataTypeFilter === dataSet.filter.type}
