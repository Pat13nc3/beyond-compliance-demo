...
import { X, Save, Shield, Globe, Layers, AlertTriangle, MessageSquare } from 'lucide-react';

// CORRECTED: The modal now accepts an `initialData` prop with a default value
const CreateRuleModal = ({ onClose, onSave, initialData = {} }) => {
    // State is now pre-filled with initialData if it exists, otherwise it uses default values
    const [ruleName, setRuleName] = useState(initialData.name || '');
    const [ruleType, setRuleType] = useState(initialData.type || 'Web2');
    const [trigger, setTrigger] = useState(initialData.trigger || 'Transaction Value');
    const [condition, setCondition] = useState(initialData.condition || 'exceeds');
    const [value, setValue] = useState(initialData.value || '');
    const [action, setAction] = useState(initialData.action || 'Create Alert');
    ...
    const handleSave = () => {
        ...
        const newRule = {
            ...
            name: ruleName,
            type: ruleType,
            context: 'Nigeria - PSP', // This would be more complex in a real app
            status: 'Inactive',
        };
        onSave(newRule);
        onClose();
        ...
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 animate-fade-in-fast">
            <div className="bg-[#1e252d] rounded-2xl shadow-2xl p-6 w-full max-w-2xl text-white">
                <div className="flex justify-between items-center mb-4 border-b border-gray-700 pb-3">
                    <h3 className="text-2xl font-bold text-[#c0933e]">Create New Rule</h3>
                ...
                <div className="space-y-4">
                    ...
                    <div className="p-4 bg-gray-800 rounded-lg">
                        ...
                        <div className="flex items-center space-x-2">
                            ...
                            <select value={trigger} onChange={(e) => setTrigger(e.target.value)} className="w-1/3 p-2 bg-gray-700 rounded-md">
                                <option>Transaction Value</option>
                                <option>User Country</option>
                                <option>Transaction Frequency</option>
                            </select>
                             <select value={condition} onChange={(e) => setCondition(e.target.value)} className="w-1/4 p-2 bg-gray-700 rounded-md">
                                <option>exceeds</option>
                                <option>is equal to</option>
                                <option>is from</option>
                            </select>
                             <input type="text" value={value} onChange={(e) => setValue(e.target.value)} placeholder="Value..." className="w-1/2 p-2 bg-gray-700 rounded-md"/>
                        </div>
                    </div>
                     <div className="p-4 bg-gray-800 rounded-lg">
                        <h4 className="font-bold mb-3 text-gray-300">Action (THEN...)</h4>
                         <div className="flex items-center space-x-2">
                            ...
                            <select value={action} onChange={(e) => setAction(e.target.value)} className="w-full p-2 bg-gray-700 rounded-md">
